...
export default function BirthSnapshotSection() {
  ...
  // Auto-regenerate chart for returning users who completed the form before
  // but lost their sessionStorage (e.g., browser restart). Without this, they
  // would land on step 3 with pre-filled data and no snapshot, forcing a manual
  // re-click of "Generate".
  useEffect(() => {
    if (autoGeneratedRef.current) return;
    if (pending) return;
    // Wait until form cache has been hydrated and all steps are complete
    if (!step1Complete || !step2Complete || !step3Complete) return;
    // Only auto-generate for users who already completed the form before
    const stored = loadBirthInput();
    if (!stored) return;
    // If a chart already exists in sessionStorage, no need to regenerate
    if (loadChartFromStorage()) return;
    // All conditions met — auto-generate the chart silently
    autoGeneratedRef.current = true;
    void handleGenerate();
    // handleGenerate is intentionally omitted — it's recreated every render
    // and we only want this to fire when form-completion state changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pending, step1Complete, step2Complete, step3Complete]);
          ...
          <>
              ...
              <div className="mt-3 flex gap-2 overflow-x-auto overscroll-x-contain pb-1 [-webkit-overflow-scrolling:touch] sm:grid sm:grid-cols-3 sm:overflow-visible sm:pb-0">
                <div
                  className={`rounded-sm border px-3 py-2 text-xs font-mono uppercase tracking-widest ${
                    step === 1
                      ? "border-gold/40 bg-gold/[0.04] text-ink"
            ...
            <Card className="mt-10 overflow-hidden border-gold/25 bg-void/60">
              ...
              <CardContent className="p-6">
                ...
                {step === 2 ? (
                  <div className="space-y-4">
                    <div className="flex items-start gap-3 rounded-sm border border-gold/10 bg-gold/[0.03] p-4">
                      <input
              ...
              <Card className="border-gold/20 bg-void/70">
                ...
                <CardContent className="grid gap-4 md:grid-cols-3">
                  {structured.strengths.map((s) => (
                    <div key={s.title} className="rounded-sm border border-gold/[0.08] bg-gold/[0.03] p-4">
